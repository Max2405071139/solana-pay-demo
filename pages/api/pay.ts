...
// CONSTANTS
const myWallet = 'H21H7Jc8bEMvkSJkh45sp7DGnT9pA7qQnDYqxsiks9pv'; // Replace with your wallet address (this is the destination where the payment will be sent)
const recipient = new PublicKey(myWallet);
const amount = new BigNumber(0.0001); // 0.0001 SOL
const label = 'BrushO Fundation';
const memo = 'Order#349057823409';
const endpoint = 'https://api.devnet.solana.com'; // Replace with your QuickNode endpoint
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    if (req.method === 'POST') {
        try {
            ...
            const message = `Order ID #0${Math.floor(Math.random() * 999999) + 1}`;
